...
# --- 初始化 ElasticSearch 相关组件 ---
# 这些对象应该只被创建一次
try:
    print(f"尝试连接到 ES 向量存储: {cfgMgmt.ES_URI}...")
    vectorstore_instance = libMgmt.connect_to_existing_vectorstore(cfgMgmt.ES_CONN_ARGS, 
                                                                   cfgMgmt.INDEX_NAME,
                                                                   cfgMgmt.EMBEDDING_MODEL_PATH, 
                                                                   cfgMgmt.DEVICE)
    print("ES 向量存储连接成功。")
except Exception as e:
    print(f"连接 ES 向量存储失败: {e}")
    print("请确保 ES 服务正在运行 (例如，通过 docker-compose)。")
...
def retrieve_top_k_documents(query: str, top_k: int = 3):
    ...
    if not vectorstore_instance or not reranker_instance:
        return ["系统未准备好：ES 数据库或 Reranker 模型未成功加载。请检查控制台错误信息。"]

    if not query:
        return ["请输入查询文本。"]

    print(f"\n接收到查询: '{query}', 正在从 ES 检索 Top {top_k} 文档...")
    try:
...
def upload_and_store_document(file_obj):
    """
    处理上传的文件，并将其存储到 ES 向量库。
    """
    if not vectorstore_instance:
        return "系统未准备好：ES 数据库未成功加载。请检查控制台错误信息。"
    ...
    try:
        ...
        print(f"文件 '{file_name}' 已上传并成功添加到 ES 向量库。")
...
with gr.Blocks() as demo:
    gr.Markdown(
        ...
        你可以输入文本查询来检索 ES 向量库中的文档，或上传新的文档以扩充知识库。
        确保 ES 服务和所有必要的模型已正确配置和运行。
        """
    )
    ...
    with gr.Row():
        ...
        with gr.Column(scale=1):
            ...
            upload_button = gr.Button("上传并存储到 ES")

    output_text = gr.Textbox(label="检索结果", lines=10, interactive=False)
    
    # 提示当前 ES 状态，实际文档数量需要通过 ES API 查询
    # gr.Markdown(
    #     f"""
    #     **ES 配置信息：**
    #     * URI: `{cfgMgmt.ES_URI}`
    #     * Collection Name: `{cfgMgmt.INDEX_NAME}`
