...
# 配置
EXCEL_PATH = 'panwei_5000_questions.xlsx'
org_quest_col_name = "用户问题"  # 原始问题列名称
RESULT_EXCEL_PATH = 'panwei_5000_questions_adversarial.xlsx'
...
if org_quest_col_name not in df.columns:
    raise ValueError(f"输入表格需要有{org_quest_col_name}列")

results = [None] * len(df)

with ThreadPoolExecutor(max_workers=len(api_keys)) as executor:
    futures = [executor.submit(process_row, idx, q) for idx, q in enumerate(df[org_quest_col_name])]
